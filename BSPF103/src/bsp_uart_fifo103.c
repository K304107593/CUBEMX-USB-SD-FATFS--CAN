/**************************************************************************
 * 模块名称 : 串口中断+FIFO驱动模块
 * FilePath: \USBCANSL\bspf103\src\bsp_uart_fifo103.c
 * Date: 2021-08-17 22:06:51
 * LastEditTime: 2021-08-21 01:03:24
 * 说     明:
 **************************************************************************/
...
static void InitHardUart(void)
{
  ...
  /* 配置NVIC the NVIC for UART */
  HAL_NVIC_SetPriority(USART4_IRQn, 2, 0);
  HAL_NVIC_EnableIRQ(USART4_IRQn);

  /* 配置波特率、奇偶校验 */
  bsp_SetUartParam(USART4, UART4_BAUD, UART_PARITY_NONE, UART_MODE_TX_RX);
#endif

#if UART5_FIFO_EN == 1 /*  串口5  */
  /*使能 GPIO TX/RX 时钟*/
  ...
  /* 配置波特率、奇偶校验 */
  bsp_SetUartParam(USART5, UART5_BAUD, UART_PARITY_NONE, UART_MODE_TX_RX);
#endif
}
...
static void UartIRQ(UART_T *_pUart)
{
  ...
  /* RXNEIE为非空中断使能,PEIE为奇偶校验错误 */
  CLEAR_BIT(_pUart->uart->CR1, (USART_CR1_RXNEIE | USART_CR1_PEIE));
  /* 帧错误、过载或者噪声错误 */
  CLEAR_BIT(_pUart->uart->CR3, USART_CR3_EIE);
  /*监测到总线空闲*/
  CLEAR_BIT(_pUart->uart->CR1, USART_CR1_IDLEIE);
...
void USART_Printf(uint8_t UsartPort, const char *fmt, ...)
{
  va_list ap;
  ...
  if (UsartPort == 1)
  {
    comSendBuf(COM1, databuf, Uart_strlen(databuf));
  }
  if (UsartPort == 2)
  {
    comSendBuf(COM2, databuf, Uart_strlen(databuf));
  }
#if 0
	if(UsartPort == 1)
	{
...
/*
*********************************************************************************************************
*	函 数 名: 主要实现不用微库实现printf和getchar
*	功能说明: 
*	形    参: 无
*	返 回 值: 无
*********************************************************************************************************
...
/*此下面的代码主要是为了不使用MicroLIB实现printf等函数*/


#pragma import(__use_no_semihosting)  //主要是为了使芯片不进入半主机模式,如果进入了半主机模式,运行的时候会卡住两三次不进入mian函数,标准库不能使用半主机模式
